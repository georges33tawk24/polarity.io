...
## Bigger than it was (230, then 320): at those sizes the dots were 2-3px on a
## phone and the map was decorative rather than usable. It can be hidden entirely
## now, so the trade for screen space is the player's to make.
##
...
func _draw() -> void:
	...
	draw_arc(centre, r - 3.0 * u, 0.0, TAU, 64, _border, 5.0 * u, true)
	draw_arc(centre, r - 8.0 * u, 0.0, TAU, 48, Color(_border, 0.30), 1.0 * u, true)
	for i in 4:
		var a := PI * 0.25 + float(i) * PI * 0.5
		var rp := centre + Vector2(cos(a), sin(a)) * (r - 9.0 * u)
		draw_circle(rp + Vector2(0, 1.0 * u), 4.5 * u, Color(0, 0, 0, 0.45))
		draw_circle(rp, 3.5 * u, Color(0.72, 0.70, 0.66, 0.75))
	...
	var scale := (r - 15.0 * u) / world_r

	# Safe zone.
	draw_arc(centre, arena.ring_radius * scale, 0.0, TAU, 40, _ring_col, 2.5 * u, true)
	...
	for m: Magnet in arena.magnets:
		...
		var dot: float = clampf(2.5 + (m.mass / maxf(player_mass, 1.0)) * 3.0, 2.5, 9.0) * u
		if m.is_leader:
	...
	if player != null and player.alive:
		...
		draw_circle(pp, 5.5 * u, Color(0.98, 0.98, 0.99))
		draw_arc(pp, 9.0 * u, 0.0, TAU, 20, Color(0.95, 0.82, 0.45, 0.95), 2.0 * u, true)
